...
<table align="center" bordercolor="#CCCCCC" border="1" height="70" cellpadding="0" cellspacing="0" bgcolor="#EEF2FB">
	<tr>
        ...
        <th width="120">是否通过认证</th>
    </tr>
    <%
    	Collection memberCollection = PublicOperator.getRequestMember();
    	if(memberCollection!=null)
    	{
	    	...
	    	%>
	    		<form action="<%=basePath %>servlet/AcceptMember?memberID=<%=member.getMemberID()%>" method="post" class="niceform" target="_self">
	    	<%
	    		out.println("<input type='submit' name='permit' value='是' />&nbsp&nbsp&nbsp");
	    	%>
	    		<form action="<%=basePath %>servlet/RefuseMember?memberID=<%=member.getMemberID()%>" method="post" class="niceform" target="_self">
